...
;						;мое корявое дополнение для прерывания чтения из компорта тк
		...
		in a,(c)		
		bit 1,a			;проверяем на переполнение буфера - часто выдает переполнение, надо проверять в вызывающей
					;проге, почему хз пока
...
.skp1		ld	(hl),a
		inc hl
		dec	de		ж уменьшаем количество контрольных байт от хэнг-апа
		jr UART_READ
